...
const Sidebar = ({ isOpen, onClose, onOpenTripModal }) => {
    ...
    return (
        <>
            ...
            ></div>
            <nav
                className={`sidebar fixed inset-y-0 left-0 w-80 !z-[9999] p-6 flex flex-col transition-transform duration-400 ease-out bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
            >
