    ...
    return (
        <header className="fixed top-0 left-0 right-0 h-14 md:h-16 bg-[var(--card-bg)] border-b border-[var(--card-border)] z-50 px-4 md:px-8 flex items-center justify-between transition-colors duration-[400ms]">
            ...
            <div className="flex items-center space-x-2">
                ...
                {activeTrip && (
                    <button onClick={() => onEditTrip(activeTrip)} className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-slate-100/20 transition-colors">
                        <i className="fa-solid fa-pen-to-square text-slate-400 hover:text-blue-500"></i>
                    </button>
                )}
            </div>
        </header>
